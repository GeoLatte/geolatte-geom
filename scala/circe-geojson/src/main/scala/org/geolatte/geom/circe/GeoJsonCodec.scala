...
object GeoJsonCodec {
  ...
  private def encodeLineStringCoordinates[P <: Position](geom: LineString[P]) : Json = geom.getPositions.asJson

  implicit def encodeLineString[P <: Position]: Encoder[LineString[P]] = new Encoder[LineString[P]] {
  ...
  implicit def encodeMultiPoint[P <: Position]: Encoder[MultiPoint[P]] = new Encoder[MultiPoint[P]] {
  ...
  implicit def encodeMultiLineString[P <: Position]: Encoder[MultiLineString[P]] = new Encoder[MultiLineString[P]] {
  ...
  implicit def encodeMultiPolygon[P <: Position]: Encoder[MultiPolygon[P]] = new Encoder[MultiPolygon[P]] {
    override def apply(geom: MultiPolygon[P]): Json =
      Json.obj(
        "type" -> Json.fromString("MultiPolygon"),
        "crs"  -> geom.getCoordinateReferenceSystem.getCrsId.asJson,
        "coordinates" -> Json.arr( geom.components()
                                     .map(encodePolygonCoordinates ):_*)
        )
  }

 implicit def encodeGeomColl[P <: Position] : Encoder[GeometryCollection[P]] = new Encoder[GeometryCollection[P]]{
